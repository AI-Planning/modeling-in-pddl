...
(define (domain music-v4)
    ...
    ;   - Make sure you increase the cheat counter at the end of this note
    ;   - Take note that we aren't using the chord as a parameter. So technically
    ;     the planner may cheat with a perfectly legit note! That's fine.
    (:durative-action play-cheat-note
        :parameters (?p - pitch ?l - length)
        :duration (= ?duration (note-length ?l))

        ; TODO: You can copy-paste from above, but just take note of
        ;       the differences listed above this action.
        :condition ()
        :effect ()
    )
    ...
    (:durative-action play-chord
        ...
        :effect (and
            ...
            ; NOTE: Check out the cheat reset here! Cool, n'est pas?
